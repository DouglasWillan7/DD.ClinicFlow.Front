...
function getInitialPhone(value: string) {
  ...
  if (!value.trim().startsWith("+")) {
    const defaultPhone = parsePhoneNumberFromString(digits, defaultCountry);
    if (defaultPhone?.isPossible()) {
      return {
        country: defaultCountry,
        nationalValue: defaultPhone.formatNational(),
      };
    }
  }
...
export const InternationalPhoneField = forwardRef<
  HTMLInputElement,
...
>(
  (
    ...
    useEffect(() => {
      ...
      lastEmittedValue.current = null;
      const nextPhone = getInitialPhone(value);
      setCountry(nextPhone.country);
      setNationalValue(nextPhone.nationalValue);
    }, [value]);
    ...
    function updateNationalValue(nextValue: string, nextCountry = country) {
      ...
      emitChange(formatter.getNumberValue() ?? "");
    }
